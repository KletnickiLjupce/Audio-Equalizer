...
class EqualizerWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # da ne treba dropdown-ot da se napolni so data?
    ...
    @pyqtSlot()
    def choose(self):
        self.filename = QFileDialog.getOpenFileName(self, 'Pick a file:')
        my_list=list(self.filename)
        my_list.pop(1)
        my_tuple=tuple(my_list)
        path=''.join(my_tuple)
        self.fs,self,wav=wav.read(path)
        
        os.system('play'+path)
#        self.fs, self.wav = wavfile.read(self.filename)
        
#        fileNameSplitList = self.filename.split('/') 
#        self.audioName = fileNameSplitList[-1]
#        self.text_filename.setText(audioName)

        
#        self.filename = r'C:\Users\ljupce\Desktop\das proekt\audio\Glas.wav'
##        self.fs, self.wav = wavfile.read(self.filename)
#        self.fs, self.wav = wavfile.read(self.filename)

#        fileNameSplitList = self.filename.split('/') 
#        self.audioName = fileNameSplitList[-1]
#        self.text_filename.setText(self.filename)
#        folder = ''
        filename = r'/home/ljupce/Desktop/audio/Glas.wav'
        self.fs, self.wav = wavfile.read(filename)
        os.system('play '+filename)
        self.wav = self.wav / 2**15
#        t = np.arange(wav.shape[0]) / self.fs
#        os.system('play '+ filename)
    ...
    def save(self):
        ...
        wavfile.write('/home/ljupce/Desktop/audio/GlasNOVII.wav', self.fs,
              np.array(self.wav_out*2**15, dtype='int16'))
#        os.system(play,'/home/ljupce/Desktop/audio/GlasNOVII.wav')
        
        filename = r'/home/ljupce/Desktop/audio/GlasNOVII.wav'
        os.system('play '+filename)
        #        self.savename = QFileDialog.getSaveFileName(self, 'Save as:')
#        wavfile.write(self.savename, self.fs , np.array(self.wav_out*2**15, dtype = 'int16'))
#        np.savetxt("wav.txt", self.wav_out)
#        self.savename = r'C:\Users\ljupce\Desktop\das proekt\audio\Novo22.wav'
##        wavfile.write(self.savename, self.fs , np.array(self.wav_out*2**15, dtype = 'int16'))
#        wavfile.write(self.savename , self.fs,
#              np.array(self.wav_out*2**15, dtype='int16'))
#        np.savetxt("wav.txt", self.wav_out)
...
#        b1,a1 = sig.iirfilter(N, f1, btype = 'low', ftype = 'butter')
        b1,a1 = sig.iirfilter(N, f1, btype = 'lowpass', ftype = 'butter')
        ...
        b2,a2 = sig.iirfilter(N, f2, btype= 'lowpass', ftype = 'butter')
        ...
        f3 = 63 / (self.fs/2)
        f4 = 100 / (self.fs/2)
        b3, a3 = sig.iirfilter (N, [f3,f4], btype = 'bandpass', ftype ='butter')   
        ...
        f5 = 100 / (self.fs/2)
        f6 = 160 / (self.fs/2)
        b4, a4 = sig.iirfilter (N, [f5,f6], btype = 'bandpass', ftype ='butter')   
        ...
        f7 = 160 / (self.fs/2)
        f8 = 250 / (self.fs/2)
        b5, a5 = sig.iirfilter (N, [f7,f8], btype = 'bandpass', ftype ='butter')   
        ...
        f9 = 250 / (self.fs/2)
        f10 = 400 / (self.fs/2)
        b6, a6 = sig.iirfilter (N, [f9,f10], btype = 'bandpass', ftype ='butter')   
        ...
        f11 = 400 / (self.fs/2)
        f12 = 630 / (self.fs/2)
        b7, a7 = sig.iirfilter (N, [f11,f12], btype = 'bandpass', ftype ='butter')   
        ...
        f13 = 630 / (self.fs/2)
        f14 = 1000 / (self.fs/2)
        b8, a8 = sig.iirfilter (N, [f13,f14], btype = 'bandpass', ftype ='butter')   
        ...
        # f[max] = [16K]
        f15 = 16000 / (self.fs/2)
        b9,a9 = sig.iirfilter(N, f15, btype = 'highpass', ftype = 'butter')
...
#        self.wav_out = self.wav1 * gain1 + self.wav2 * gain2  + self.wav3 * gain3 + self.wav4 * gain4+self.wav5 * gain5 +self.wav6 * gain6 + self.wav7 * gain7 + self.wav8 * gain8 + self.wav9 * gain9 + self.wav10 * gain10 + self.wav11 * gain11+self.wav12 * gain12 + self.wav13 * gain13 + self.wav14 * gain14 + self.wav15 * gain15
        self.wav_out = self.wav9 *gain9+ self.wav10* gain10 + self.wav11 * gain11+self.wav12 * gain12 + self.wav13 * gain13 + self.wav14 * gain14 + self.wav15 * gain15
        #normalizacija
    ...
    def preset(self):
        ...
        if((self.ui.dropdown_presets.currentIndex() == 4)):
            self.clasical()
        if((self.ui.dropdown_presets.currentIndex() == 5)):
            self.pop()
    ...
    def pop(self):
        ...
        self.ui.slider_100.setValue(3);
        self.ui.slider_160.setValue(4);
        self.ui.slider_250.setValue(-12);
        self.ui.slider_400.setValue(12);
        self.ui.slider_630.setValue(0);
        self.ui.slider_1k.setValue(2);
        self.ui.slider_1k6.setValue(6);
        self.ui.slider_2k5.setValue(-10);
